/*
 * Copyright 2024 CloudWeGo  3dman.cn
 */
...
func formatBrowserToolPrompt(ctx context.Context, bs *browseruse.BrowserState) (*schema.Message, error) {
	...
	if len(bs.Screenshot) > 0 {
		...
		resp, err := chatModel.Generate(ctx, []*schema.Message{
			schema.SystemMessage("你是一个图像分析助手，请详细描述图片中的内容，包括主体、背景、颜色、动作等细节。"),
		...
		message = schema.AssistantMessage(fmt.Sprintf("website content is :%s", resp.Content), nil)
		fmt.Println(resp)
	}
	return message, nil
}
